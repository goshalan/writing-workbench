...
5. Invoke the local writing agent only after an explicit user action.
...
│ chapters/backups │  │ local Hermes / mock     │
└───────────────────┘  └─────────────────────────┘
```
...
- whole-manuscript relationship, foreshadowing, continuity, and next-chapter analysis;
- automatic English or Simplified Chinese UI selection from browser language preferences;
...
The HTTP layer parses JSON, applies the configured request-size limit, validates required fields, calls a domain service, and maps expected failures to stable JSON errors. It does not expose the Hermes command, profile path, prompts, process output, or absolute manuscript paths.
...
- `hermes`: invokes an isolated local profile using the local Hermes executable;
...
Provider selection is fixed at process startup. A browser request cannot change the Hermes executable or profile. Hermes runs use bounded timeouts, ignore workspace rules, expose only the non-mutating vision toolset, and treat manuscript text as untrusted content. Failures become structured `502`, `503`, or `504` responses without returning prompts or raw process output.
...
### Server to local Hermes

Only an explicit rewrite, question, or whole-manuscript analysis starts a Hermes subprocess. Rewrites receive a selected passage and nearby context; questions receive the current chapter and recent conversation; analysis receives bounded copies of saved chapters. The full-book endpoint deliberately excludes unsaved browser buffers. Writing Workbench does not accept or store API keys.
...
- disabled, mock, and local Hermes provider behavior;
- absence of local-agent paths, prompts, and raw failures in responses.
